...
export const metadata: Metadata = { title: 'DMO debt review queue' }
export const dynamic = 'force-dynamic'

export default async function DmoReviewQueuePage() {
  const result = await getPendingDmoReviews()
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-12 lg:px-8 lg:py-16">
      <PageHeader
        ...
        title="State and FCT debt evidence awaiting human action"
        description="Archived DMO sources land here once deterministic extraction completes. Approval and publication into the governed debt ledger are separate controlled actions."
      />
      ...
      {result.error ? (
              ...
              <Card key={item.source_document_id}>
                <CardHeader>
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      <CardTitle>
                        {humanize(item.debt_kind)} debt · as at{' '}
                ...
                <CardContent>
                  ...
                  <div className="border-border mt-5 flex flex-wrap items-center justify-between gap-3 border-t pt-4">
                    ...
                    <Button asChild size="sm">
                      <Link href={`/review/dmo/${item.source_document_id}`}>
                        {item.approved
        ...
        <Card className="mt-8 border-dashed">
          <CardContent className="pt-6">
            <CheckCircle2 className="text-primary size-5" aria-hidden="true" />
            <p className="mt-3 font-medium">DMO queue clear</p>
            <p className="text-muted-foreground mt-2 max-w-2xl text-sm leading-6">
              There are no unpublished DMO debt sources awaiting human action.
        ...
        <Card className="mt-6 border-amber-300 bg-amber-50">
          <CardContent className="flex items-start gap-3 pt-6 text-amber-950">
            ...
            <p className="text-sm font-medium">
              ...
              extracted yet - check the DMO collector before assuming
