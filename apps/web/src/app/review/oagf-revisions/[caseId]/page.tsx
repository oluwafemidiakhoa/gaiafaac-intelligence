...
export default async function OagfRevisionPacketPage({
  ...
  async function resolveAction(formData: FormData) {
    ...
    const action = await resolveOagfRevision(
      caseId,
      reviewerId,
      resolutionCode,
      note,
    ...
    return (
      <div className="mx-auto max-w-7xl px-5 py-12 lg:px-8 lg:py-16">
        <PageHeader
          ...
          title="Revision packet unavailable"
          description="Only retained OAGF revision cases can be opened here."
        />
        <div className="mt-8">
          <DataUnavailable
            message={result.error ?? 'Revision packet unavailable.'}
          />
        </div>
        <Button asChild variant="outline" className="mt-5">
          <Link href="/review/oagf-revisions">Back to revisions</Link>
        </Button>
      </div>
    )
  }
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-12 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardTitle className="pt-3">Version lineage</CardTitle>
            <CardDescription>
              Official OAGF bytes changed for the same publication identity.
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div>
                ...
                <dd className="mt-1 font-medium">
                  {item.revenue_month
                    ...
                    : 'Not linked to a governed period'}
                </dd>
              </div>
              <div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <p className="text-muted-foreground">
              Published allocations and fiscal proofs remain untouched until a
              separate governed replacement workflow is explicitly performed.
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Previous official version</CardTitle>
            <CardDescription className="font-mono break-all">
              SHA-256 {item.previous_sha256}
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-wrap gap-3">
            <Button asChild variant="outline">
              <a
                href={`/review/oagf-revisions/${item.id}/source/previous`}
                target="_blank"
              >
                Open retained bytes
        ...
        <Card>
          <CardHeader>
            <CardTitle>Current official version</CardTitle>
            <CardDescription className="font-mono break-all">
              SHA-256 {item.current_sha256}
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-wrap gap-3">
            <Button asChild variant="outline">
              <a
                href={`/review/oagf-revisions/${item.id}/source/current`}
                target="_blank"
              >
                Open retained bytes
                <ExternalLink className="size-4" />
      ...
      <Card className="mt-8">
        <CardHeader>
          <CardTitle>Classify official revision</CardTitle>
          <CardDescription>
            Record what the human reviewer determined after comparing both
            retained versions.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form action={resolveAction} className="grid gap-4 lg:max-w-2xl">
            ...
            <label className="grid gap-1.5 text-sm font-medium">
              ...
              >
                ...
                <option value="metadata_only_change">
                  Metadata/layout-only change
                </option>
                <option value="no_material_fiscal_change">
                  No material fiscal change
                </option>
                <option value="requires_data_republication">
                  Fiscal values changed — republication required
                </option>
            ...
            <label className="flex items-start gap-3 text-sm leading-6">
              ...
              />
              <span>
                I compared the retained previous and current official OAGF
                source versions. I understand this classification does not
                silently overwrite published fiscal evidence.
              </span>
            </label>
            <Button type="submit" disabled={reviewers.length === 0}>
              Record revision classification
            </Button>
          </form>
        </CardContent>
      </Card>
