...
export default async function OagfRevisionQueuePage() {
  ...
  const investigation = cases.filter(
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        <Card>
          <CardHeader>
            ...
            <CardTitle className="pt-3 text-2xl">{cases.length}</CardTitle>
            <CardDescription>Open revision cases</CardDescription>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader>
            <CheckCircle2 className="text-primary size-5" />
            <CardTitle className="pt-3 text-2xl">{pending}</CardTitle>
        ...
        <Card>
          <CardHeader>
            ...
            <CardTitle className="pt-3 text-2xl">{investigation}</CardTitle>
            <CardDescription>
              Investigation / republication required
            </CardDescription>
          </CardHeader>
        </Card>
      </div>
      ...
      {result.error ? (
        <Card className="mt-8 border-dashed">
          <CardContent className="pt-6">
            <p className="font-medium">Revision service unavailable</p>
            ...
            <Card key={item.id}>
              <CardHeader>
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <CardTitle>{item.reporting_label ?? item.title}</CardTitle>
                    <CardDescription className="mt-2">
                      ...
                      UTC
                      {item.revenue_month
                        ? ` · reporting period ${formatDate(item.revenue_month)}`
                        : ''}
                    </CardDescription>
                  </div>
                  <StatusPill
              ...
              <CardContent>
                <dl className="grid gap-4 text-sm md:grid-cols-3">
                  <div>
                    <dt className="text-muted-foreground">Version change</dt>
                    <dd className="mt-1 font-mono font-semibold">
                      v{item.previous_version} → v{item.current_version}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-muted-foreground">Previous SHA</dt>
                    <dd className="mt-1 font-mono text-xs">
                      {item.previous_sha256.slice(0, 16)}…
                    </dd>
                  </div>
                  <div>
                    <dt className="text-muted-foreground">Current SHA</dt>
                    <dd className="mt-1 font-mono text-xs">
                      {item.current_sha256.slice(0, 16)}…
                    </dd>
                  </div>
                </dl>
                <div className="border-border mt-5 flex justify-end border-t pt-4">
                  <Button asChild size="sm">
                    <Link href={`/review/oagf-revisions/${item.id}`}>
                      Open revision packet
        ...
        <Card className="mt-8 border-dashed">
          <CardContent className="pt-6">
            <CheckCircle2 className="text-primary size-5" />
            <p className="mt-3 font-medium">No unresolved OAGF revisions</p>
            <p className="text-muted-foreground mt-2 text-sm">
              A changed official historical PDF will appear here automatically
              after the revision monitor runs.
            </p>
          </CardContent>
        </Card>
      )}
