...
import {
  ...
  getIgrReviewPacket,
  publishIgrReview,
} from '@/lib/nbs-igr-review-api'
import { getReviewActors } from '@/lib/review-api'

export const metadata: Metadata = { title: 'NBS IGR review packet' }
export const dynamic = 'force-dynamic'

export default async function IgrReviewPacketPage({
  params,
  searchParams,
}: {
  ...
  async function approveAction(formData: FormData) {
    ...
    if (!reviewerId || !attestation) {
      redirect(
        `/review/nbs-igr/${encodeURIComponent(sourceId)}?error=${encodeURIComponent('Choose the actual reviewer and complete the attestation.')}`,
      )
    }
    const action = await approveIgrReview(sourceId, reviewerId)
    if (action.error) {
      redirect(
        `/review/nbs-igr/${encodeURIComponent(sourceId)}?error=${encodeURIComponent(action.error)}`,
      )
    }
    redirect(`/review/nbs-igr/${encodeURIComponent(sourceId)}`)
  }
  ...
  async function publishAction(formData: FormData) {
    ...
    if (!publisherId || !attestation) {
      redirect(
        `/review/nbs-igr/${encodeURIComponent(sourceId)}?error=${encodeURIComponent('Choose a publisher and complete the publication attestation.')}`,
      )
    }
    const action = await publishIgrReview(sourceId, publisherId)
    if (action.error) {
      redirect(
        `/review/nbs-igr/${encodeURIComponent(sourceId)}?error=${encodeURIComponent(action.error)}`,
      )
    }
    redirect('/review/nbs-igr')
  }
    ...
    return (
      <div className="mx-auto max-w-7xl px-5 py-12 lg:px-8 lg:py-16">
        <PageHeader
          ...
          description="Only real, archived NBS IGR sources can be opened here."
        />
        <div className="mt-8">
          <DataUnavailable
        ...
        <Button asChild variant="outline" className="mt-5">
          <Link href="/review/nbs-igr">Back to NBS IGR queue</Link>
        </Button>
      </div>
    )
  }
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-12 lg:px-8 lg:py-16">
      <PageHeader
        eyebrow="NBS evidence control"
        title={`${packet.fiscal_year} annual IGR`}
        description="Review retained NBS IGR evidence, approve it explicitly, then publish only through a different active administrator."
      />
          ...
          <table className="w-full min-w-4xl border-collapse text-left text-sm">
            <thead className="bg-muted/50">
              <tr className="border-border border-b">
                ...
                <th className="px-4 py-3 font-medium">IGR amount</th>
            ...
            {packet.published ? (
              <div className="space-y-2 text-sm">
                ...
                <p className="text-muted-foreground">
                  This NBS IGR evidence is live in the governed ledger.
                </p>
              </div>
            ) : (
              <form action={publishAction} className="space-y-4">
                ...
                <label className="flex items-start gap-3 text-sm leading-6">
                  ...
                  />
                  <span>
                    I am publishing previously approved NBS IGR evidence under
                    four-eyes control.
                  </span>
                </label>
                <Button
