...
export default async function CreditCommitteePackPage({
  searchParams,
}: PageProps) {
    ...
    <div className="gaia-shell py-12 lg:py-16">
      ...
      <Card className="mt-8">
        <CardHeader>
          ...
          <CardDescription>
            Enter a Gaia state slug and evidence year. The pack uses the same
            published Decision Packet source of truth rather than duplicating
            calculations.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form
            method="get"
            className="grid gap-4 sm:grid-cols-[1fr_9rem_auto]"
          >
      ...
      {state && !packet ? (
        <div className="mt-8">
          <DataUnavailable
            message={
              result?.error ??
              'No governed evidence pack is available for this selection.'
            }
          />
        </div>
      ) : null}
        ...
        <div className="mt-8 space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Evidence status: {packet.evidence_status} ·{' '}
                {packet.coverage_label}
              </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div>
                ...
                <p className="mt-1 font-mono font-semibold">
                  {pct(packet.deduction_burden_pct)}
                </p>
              </div>
              <div>
                <p className="text-muted-foreground text-xs uppercase">
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Published allocation evidence only.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3 text-sm">
                <p>
                  Momentum: <strong>{packet.momentum}</strong>
                  {packet.momentum_pct === null
                    ? ''
                    : ` · ${pct(packet.momentum_pct)}`}
                </p>
                <p>
                  Volatility: <strong>{packet.volatility}</strong>
                  {packet.volatility_cv_pct === null
                    ? ''
                    : ` · CV ${pct(packet.volatility_cv_pct)}`}
                </p>
                <p className="text-muted-foreground">
                  No missing month is inferred or annualized.
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Kept separate from FAAC to avoid mixing evidence domains.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3 text-sm">
          ...
          <div className="grid gap-5 lg:grid-cols-3">
            {[
              [
                'Debt & debt service',
                'Not asserted unless a comparable governed debt lane is available for this jurisdiction and period.',
              ],
              [
                'Budget & expenditure',
                'Not asserted unless published governed budget/expenditure evidence is available for the same review boundary.',
              ],
              [
                'Peer comparison',
                'Use Gaia Compare for a declared peer set; the pack does not choose peers or manufacture a benchmark automatically.',
              ],
            ].map(([title, body]) => (
              <Card key={title}>
                <CardHeader>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Observed fiscal-watch events for this evidence year.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {packet.watch_events.length ? (
                ...
                <p className="text-muted-foreground text-sm">
                  No governed watch event is recorded for this selection.
                </p>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                The pack makes unavailable evidence visible instead of filling
                it.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4 text-sm">
              ...
              <p className="text-muted-foreground">
                Analyst interpretation and reviewer sign-off belong in a
                Decision Room, separate from the evidence objects above.
              </p>
              <div className="flex flex-wrap gap-3">
                <Button asChild>
                  <Link href="/decision-rooms">
                    Save review in a Decision Room
                  </Link>
                </Button>
                <Button asChild variant="outline">
                  <Link
                    href={`/compare?states=${encodeURIComponent(packet.state_slug)}`}
                  >
                    Open peer comparison
                  </Link>
                </Button>
                <Button asChild variant="outline">
                  <Link href="/watch-contracts">Configure monitoring</Link>
                </Button>
              </div>
            </CardContent>
          </Card>
