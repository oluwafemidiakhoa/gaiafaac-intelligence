...
export function GaiaWorkflowActions({
  ...
  const isScenario =
    /(shock|reduction|increase|decrease|scenario|what if|20%|10%)/i.test(
      normalized,
    )
  ...
  return (
    <section className="border-border bg-muted/20 rounded-xl border p-5">
      ...
      <p className="text-muted-foreground mt-2 text-sm leading-6">
        Gaia can route this evidence into a deterministic institutional
        workflow. These actions do not change the evidence or silently create an
        analyst opinion.
      </p>
      <div className="mt-4 flex flex-wrap gap-2">
        {uniqueStates.length >= 2 ? (
          <Button asChild size="sm" variant="outline">
            <Link
              href={`/compare?states=${encodeURIComponent(uniqueStates.join(','))}`}
            >
              Open comparison
            </Link>
          </Button>
        ...
        {primaryState ? (
          <Button asChild size="sm" variant="outline">
            <Link
              href={`/credit-committee-pack?state=${encodeURIComponent(primaryState)}&year=${year}`}
            >
        ...
        {isScenario && primaryState ? (
          <Button asChild size="sm" variant="outline">
            <Link
              href={`/fiscal-design?state=${encodeURIComponent(primaryState)}&year=${year}`}
            >
